...
export const initSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("create-room", async ({ skillId }) => {
      ...
      if (!room) room = await Room.create({ skillId });
    ...
    socket.on("send-group-message", async ({ roomId, senderId, text }) => {
      const newMessage = await Message.create({ RoomId: roomId, senderId, text });
      const populated = await newMessage.populate("senderId");

      io.to(roomId).emit("receive-group-message", populated);
    });

    socket.on("upload", async ({ buffer, fileName, roomId, senderId }) => {
      const temp = path.join("public", `${Date.now()}-${fileName}`);
      fs.writeFileSync(temp, Buffer.from(buffer));

      const uploadRes = await uploadCloudinary(temp);
