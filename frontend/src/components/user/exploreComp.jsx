...
const ExploreComp = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      // ✅ Step 2: Fetch skills & categories (conditionally based on isAuth)
      try {
        const skillUrl = `${BASE_URL}/skill/all-skill${isAuth && user?.user?._id ? `/${user.user._id}` : ''}`;
        const skillsRes = await axios.get(skillUrl);

        const categoriesRes = await axios.get(`${BASE_URL}/category`);
        setSkills(skillsRes.data.data);
        setFilteredSkills(skillsRes.data.data);
        setCategories(categoriesRes.data.categories);
      } catch (error) {
  ...
  const handleConnect = async (skillId, providerId) => {
    try {
      const response = await axios.post(
        `${BASE_URL}/connection/request/${user?.user._id}`,
        { providerId, skillId },
        {
          headers: {
            Authorization: `Bearer ${user?.accessToken || ''}`
